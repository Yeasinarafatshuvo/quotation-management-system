...
if (!function_exists('getUserSoldPersonName')) {
    function getUserSoldPersonName($id)
    {
...
if (!function_exists('getCustomerAddress')) {
    function getCustomerAddress($id)
    {
        ...
        $shipping_address_form_order_table = Order::select('shipping_address')->where('user_id',$id)->get();
        if(!$shipping_address_form_order_table){
            $address;
            foreach ($shipping_address_form_order_table as $key => $item_address) {
                if($item_address->shipping_address != null){
                    $address = $item_address->shipping_address;
                    break;
                }
            }
            $address_string = explode(",",$address)[2];
            return substr($address_string, 11);
         
        }else{
            return Address::where('user_id', $id)->pluck('address')->first();
        }
    ...
    if (!function_exists('getProductName')) {
          function getProductName($id){
            if(is_string($id) == 1){
                return Product::where('id', intval($id))->pluck('name')->first();
            }else
            return Product::where('id', $id)->pluck('name')->first();
        }
    }

    if (!function_exists('getProductWarrantyInfo')) {
        function getProductWarrantyInfo($id){
          return Product::where('id', $id)->pluck('product_warranty')->first();
      }
  }

    if (!function_exists('getSupplierName')) {
        function getSupplierName($id){
          return Supplier::where('id', $id)->pluck('name')->first();
      }
    }
    if (!function_exists('getOrderInvoice')) {
        function getOrderInvoice($id){
          $combined_order_id =  Order::where('id', $id)->pluck('combined_order_id')->first();
          return CombinedOrder::where('id', $combined_order_id)->pluck('code')->first();
      }
    }
    ...
    if (!function_exists('getCustomerNameOrderWise')) {
        function getCustomerNameOrderWise($id){
          $user_id =  Order::where('id', $id)->pluck('user_id')->first();
          return User::where('id', $user_id)->pluck('name')->first();
      }
    }
    if (!function_exists('getProduct_sold_by_name')) {
        function getProduct_sold_by_name($id){
          return User::where('id', $id)->pluck('name')->first();
      }
    }

    if (!function_exists('getUserTotalOrders')) {
        function getUserTotalOrders($user_id)
        {
            $total_user_order = Order::where('user_id',$user_id)
    ...
    if (!function_exists('getUserName')) {
        ...
        return User::where('id', $id)->pluck('name','email','phone')->first();
      }
    }

    if (!function_exists('getEmployeeSalary')) {
        function getEmployeeSalary($employee_id){
        return User::where('employee_id', (int) $employee_id)->pluck('employee_salary')->first();
      }
    }

    if (!function_exists('getEmployeeDesignation')) {
        function getEmployeeDesignation($employee_id){
        return User::where('employee_id', (int) $employee_id)->pluck('designation')->first();
      }
    }

    if (!function_exists('getSupplierNameByPurchaseId')) {
        function getSupplierNameByPurchaseId($purchaseOrderId){
            $get_supplier_id = PurchaseDetail::select('supplier_id')->where('invoice_numbers', $purchaseOrderId)->first();
            return Supplier::where('id', $get_supplier_id->supplier_id)->pluck('name')->first();
      }
    }
